...
WDL_FEN_SKIPPING = True
EPOCH_SIZE = 1
VAL_SIZE = 5


# using data loader from the Gary Linscott (SF NNUE) https://github.com/glinscott/nnue-pytorch/blob/master/train.py
def make_data_loaders(train_filename, val_filename, feature_set_name, num_workers, batch_size, filtered, random_fen_skipping, wld_filtered, main_device, epoch_size, val_size):
  ...
  val_infinite = nnue_dataset.SparseBatchDataset(feature_set_name, val_filename, batch_size, filtered=filtered,
...
def main():
  ...
  train, val = make_data_loaders(train_filename, val_filename, FEATURE_SET_NAME, NUM_WORKERS, BATCH_SIZE, SMART_FEN_SKIPPING, RANDOM_FEN_SKIPPING, WDL_FEN_SKIPPING, device, EPOCH_SIZE, VAL_SIZE)
  ...
  trainer = pl.Trainer(logger=logger, max_epochs=EPOCH_SIZE, accelerator='gpu', devices=1)
